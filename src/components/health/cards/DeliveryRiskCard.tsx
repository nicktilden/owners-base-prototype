/**
 * DELIVERY RISK CARD
 * Shows delivery-category KPIs: RFIs at risk, submittals at risk, invoice status.
 * Delivery risk reflects downstream procurement and approval blockers.
 */
...
export default function DeliveryRiskCard({ scope = 'portfolio', projectId }: Props) {
  ...
  const projectKPIs = useMemo(() => {
    ...
    return source.map(p => {
      ...
      const deliveryKPIs = allKPIs.filter(k => k.category === 'delivery' && config.activeKPIs.includes(k.key as any));
      const worstStatus = deliveryKPIs.some(k => k.status === 'red') ? 'red' : deliveryKPIs.some(k => k.status === 'yellow') ? 'yellow' : 'green';
      return { project: p, kpis: deliveryKPIs, worstStatus };
  ...
  return (
    <HubCardFrame
      title="Delivery Risk"
      infoTooltip="Overdue RFIs, submittals, and invoice approval blockers per project."
      actions={
    ...
    >
      {displayed.length === 0
        ? <HubCardEmptyState title="No Delivery KPIs to Display" body="No active delivery KPIs are configured for this project. Visit Account Settings to enable KPIs." />
