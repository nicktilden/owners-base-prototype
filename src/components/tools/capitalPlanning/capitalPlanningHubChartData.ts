...
function rollupProgramMonths(
  planned: readonly number[],
  actual: readonly number[]
): HubPlannedCostPoint[] {
  const labels = getProgramForecastMonthLabels();
...
export function buildCapitalPlanningHubChartSeries(
  ...
  const rolled =
    periodView === "Quarter"
      ...
      : periodView === "Month"
        ? rollupProgramMonths(pMasked, actualPast)
        : rollupYears(pMasked, actualPast);

  const rolledWithYearScopedActuals =
    periodView === "Years"
      ? rolled.map((point) => {
          const year = fiscalYearFromHubYearLabel(point.label);
          if (year == null) return point;
          if (year > anchor.getFullYear()) {
            return { ...point, actual: 0 };
          }
          if (year === anchor.getFullYear()) {
            // Current FY keeps a smaller realized share than fully past FYs.
            return { ...point, actual: Math.round(point.planned * 0.1) };
          }
          // Past FYs show a concrete realized amount relative to planned.
          return { ...point, actual: Math.round(point.planned * 0.25) };
        })
      : rolled;
  ...
  if (isFullHorizon) return rolledWithYearScopedActuals;

  const trimmed = filterHubPointsByTimeline(rolledWithYearScopedActuals, periodView, rs, re, fiscalYearStartMonth);
  return trimmed.length > 0 ? trimmed : rolledWithYearScopedActuals;
}
